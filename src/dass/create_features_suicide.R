...
trigrams <- trigrams_tf_idf %>%
  # Clean the trigrams
  mutate(
    remove = if_else(str_detect(trigram, regex("\\d|^amp|amp$| amp |tl dr|straight white male")), 1, 0)
  ) %>%
  filter(remove == 0) %>%
  slice_head(n = 1666) %>%
  select(trigram)
...
# Save to file
write_csv(trigrams, "data/cleaned/dass/ngrams/trigrams_suicide.csv")

#* GET BIGRAMS ------------------------------------------------------------

# Generate and clean bigrams
bigrams_df <- suicide_df %>%
...
# Save to file
write_csv(bigrams, "data/cleaned/dass/ngrams/bigrams_suicide.csv")

#* GET UNIGRAMS -----------------------------------------------------------

# Get unigrams
unigrams_df <- suicide_df %>%
  # Find the unigrams
...
unigrams_pos <- unigrams_tf_idf %>%
  ...
  mutate(remove = if_else(str_detect(word, regex("^\\d*$|\\d\\df|\\d\\dyo|\\dish|\\d'\\d|^\\d*\\.\\d*$|skype|\\dam|	
camel's|idk|idek|cis|wan't|freshmen|ftm|'s$|i̇|don‘t|tranny|me.i|ldr$|pornography|mtf$|abt$|asd$|gcses$|tms|iop|it.i|a’s$|mart")), 1, 0)) %>%
  filter(remove == 0)
...
unigrams <- anti_join(unigrams_pos, unigrams_neg) %>%
  ...
  slice_head(n = 1667) %>%
  select(unigram = word) %>%
  bind_rows(data.frame(unigram = c("suicide", "suicidial"))) %>%
  distinct(unigram)

# Write to file 
write_csv(unigrams, "data/cleaned/dass/ngrams/unigrams_suicide.csv")
