...
# Save to file
write_csv(trigrams, "data/cleaned/dass/ngrams/trigrams_anxiety.csv")

#* GET BIGRAMS ------------------------------------------------------------

# Generate and clean bigrams
bigrams_df <- anxiety_df %>%
...
# Save to file
write_csv(bigrams, "data/cleaned/dass/ngrams/bigrams_anxiety.csv")

#* GET UNIGRAMS -----------------------------------------------------------

# Get unigrams
unigrams_df <- anxiety_df %>%
  # Find the unigrams
...
unigrams_pos <- unigrams_tf_idf %>%
  ...
  mutate(remove = if_else(str_detect(word, regex("^\\d*$|\\d\\df|\\d\\dyo|\\dish|\\d'\\d|^\\d*\\.\\d*$|skype")), 1, 0)) %>%
  filter(remove == 0)
...
# Write to file 
write_csv(unigrams, "data/cleaned/dass/ngrams/unigrams_anxiety.csv")
