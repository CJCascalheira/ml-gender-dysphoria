...
with open(my_path + '\doc\dass_output.txt', 'a') as f:
    ...
    print('Average accuracy: %.3f (%.3f)' % (np.mean(scores['test_accuracy']), np.std(scores['test_accuracy'])), file=f)
    print('Average precision: %.3f (%.3f)' % (np.mean(scores['test_precision']), np.std(scores['test_precision'])), file=f)
